...
- **test/internal/target/**: shared listener setup for the Go live-test targets. `Addr()` resolves the bind address (loopback by default, widened only via `BIND_HOST`) and `Server()` applies the read-header and companion timeouts. The four Go targets above each carried a byte-identical copy of both before this was consolidated, so the loopback default — a security control for these unauthenticated fixtures — is now asserted in one place instead of four.
