...
Supporting scripts: `test/install-chrome.sh` provisions a real non-snap Chrome for containers (Ubuntu's `chromium-browser` is a snap stub that cannot run in-container), installing it from Google's apt repo with the signing key pinned by fingerprint and the repo removed again afterwards. `test/preflight-selftest.sh`, `test/test-runner-args.sh`, and `test/install-chrome-selftest.sh` are the CI-run regression guards for the setup preflight, the target-group/dispatch wiring, and the installer's non-privileged surface.
