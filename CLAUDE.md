...
- **live-tests.yml**: A single **test** job runs all targets via `test/run-live-tests.sh --group offline` then `--group live`. Target groups (`OFFLINE_TARGETS` / `LIVE_TARGETS`) are defined in `test/run-live-tests.sh` — the workflow references groups, not individual targets, so adding a target means editing one array in the runner. A drift-guard step (`test/test-runner-args.sh`) fails CI if a dispatch target is missing from these arrays. Uses the stable Chrome shipped with the ubuntu-24.04 runner. Runs on every PR by default; add the `skip-live-tests` label to bypass. Always runs on push to `main` and `workflow_dispatch`.
