...
- **live-tests.yml**: Two parallel jobs — **offline-tests** (imports, deterministic generates, classifier/spec edge cases; no services or Chrome needed) and **live-tests** (boots REST, SOAP, GraphQL, and concat-SPA target services; uses the stable Chrome shipped with the ubuntu-24.04 runner for the rod headless backend; runs crawl-based end-to-end tests). Label-gated on PRs: add the `live-tests` label to trigger. Always runs on push to `main` and `workflow_dispatch`.
