...
MANAGED_SERVICES="rest-api soap-service graphql-server grpc-server concat-spa forms-target"
...
DEFAULT_FORMS_TARGET_PORT=8994

# All available targets
ALL_TARGETS="rest-api,soap-service,graphql-server,grpc-server,concat-spa,forms-target"
...
build_forms_target() {
    log_info "Building forms-target..."
    cd "${SCRIPT_DIR}/forms-target"
    go build -o forms-target .
    log_ok "Built test/forms-target/forms-target"
}
...
service_binary() {
    case "$1" in
        ...
        forms-target) echo "forms-target" ;;
        *)            echo "" ;;
    esac
}
...
service_default_port() {
    case "$1" in
        ...
        forms-target)   echo "$DEFAULT_FORMS_TARGET_PORT" ;;
        *)              echo "" ;;
    esac
}
...
usage() {
    ...
    echo "                     Valid: rest-api,soap-service,graphql-server,grpc-server,concat-spa,forms-target"
...
main() {
    ...
    for target in "${TARGET_ARRAY[@]}"; do
        case "$target" in
            ...
            forms-target)    build_forms_target ;;
            *)
    ...
    for target in "${TARGET_ARRAY[@]}"; do
        case "$target" in
            ...
            concat-spa)
                ...
                ;;
            forms-target)
                resolve_port_or_die forms-target "$DEFAULT_FORMS_TARGET_PORT"
                FORMS_TARGET_PORT=$RESOLVED_PORT
                log_ok "forms-target: port ${FORMS_TARGET_PORT}"
                start_forms_target "$FORMS_TARGET_PORT" || start_failed=1
                ;;
        esac
    done
