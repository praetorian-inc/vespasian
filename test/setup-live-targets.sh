...
ALL_TARGETS="rest-api,soap-service,graphql-server,grpc-server"
...
usage() {
    ...
    echo "                     Valid: rest-api,soap-service,graphql-server,grpc-server"
...
main() {
    ...
    # ── Write config ──
    write_config "${REST_API_PORT:-}" "${SOAP_SERVICE_PORT:-}" "${GRAPHQL_SERVER_PORT:-}" "${GRPC_SERVER_PORT:-}" "$targets"
